...
export default function Geography() {
  ...
  // Карта "проявляется" один раз, когда секция попадает в вьюпорт
  useEffect(() => {
  ...
  // Бегунок слева синхронизируется с активным пунктом списка
  useEffect(() => {
    const measure = () => {
    ...
    measure()
    // повторный замер после раскрытия описания активного пункта (transition ~300ms)
              ...
              >
                ...
                <g ref={mapGroupRef}>
                  {/* Материковая часть России — реальные очертания береговой линии,
                      ...
                      Дорисовывается линией при первом появлении в вьюпорте. */}
                  <path
                    pathLength={1000}
                    ...
                    style={{
                      ...
                      transition:
                        'stroke-dashoffset 2.2s cubic-bezier(0.16,1,0.3,1), fill-opacity 1.3s ease 0.7s',
                    }}
                  />
                  ...
                  <path
                    ...
                    style={{
                      ...
                      fillOpacity: revealed ? 1 : 0,
                      transition:
                        'stroke-dashoffset 1.1s cubic-bezier(0.16,1,0.3,1) 0.3s, fill-opacity 1s ease 1.1s',
                    }}
                  />
                  ...
                  {/* Азербайджан (материк + Нахичеванская эксклава), та же проекция, что и у России */}
                  <path
                    pathLength={1000}
                    d="M 374.5 516.3 L 377.6 517.2 L 378.8 513.8 L 383.0 508.5 L 386.6 515.5 L 390.2 524.8 L 393.4 525.5 L 395.6 529.0 L 389.8 530.1 L 388.6 540.3 L 387.4 545.0 L 384.9 548.0 L 385.0 554.6 L 383.3 555.2 L 378.9 548.3 L 381.4 541.8 L 379.3 537.9 L 376.7 538.9 L 368.4 548.6 L 368.3 539.5 L 365.1 537.3 L 362.1 533.7 L 364.1 529.5 L 360.4 525.0 L 361.8 521.7 L 359.1 519.4 L 357.7 515.9 L 359.4 513.8 L 364.6 517.6 L 368.4 518.4 L 369.3 516.8 L 365.9 509.7 L 367.7 507.8 L 369.7 508.3 L 374.5 516.3 Z"
                    fill="url(#landGrad)"
                    stroke="url(#landStroke)"
                    strokeWidth="1.5"
                    strokeLinejoin="round"
                    style={{
                      ...
                      fillOpacity: revealed ? 1 : 0,
                      transition:
                        'stroke-dashoffset 1.1s cubic-bezier(0.16,1,0.3,1) 0.4s, fill-opacity 1s ease 1.2s',
                    }}
                  />
                  <path
                    pathLength={1000}
                    d="M 357.9 535.8 L 360.0 539.4 L 363.1 539.3 L 363.0 541.4 L 365.9 549.0 L 361.1 547.3 L 357.5 541.2 L 356.4 536.2 L 357.9 535.8 Z"
                    fill="url(#landGrad)"
                    stroke="url(#landStroke)"
                    strokeWidth="1.5"
                    strokeLinejoin="round"
                  ...
                  {/* Линии широты */}
                  <path
                    d="M 280 200 Q 450 185 650 190 Q 850 200 1000 240"
                    fill="none"
                    stroke="#222"
                    strokeWidth="1"
                    strokeDasharray="4 8"
                    style={{ opacity: revealed ? 0.6 : 0, transition: 'opacity 1s ease 1.8s' }}
                  />
                  <path
                    d="M 250 320 Q 450 310 650 315 Q 850 330 980 360"
                    fill="none"
                    stroke="#222"
                    strokeWidth="1"
                    strokeDasharray="4 8"
                    style={{ opacity: revealed ? 0.5 : 0, transition: 'opacity 1s ease 1.9s' }}
                  />
                  <path
                    d="M 300 450 Q 480 440 650 445"
                    fill="none"
                    stroke="#222"
                    strokeWidth="1"
                    strokeDasharray="4 8"
                    style={{ opacity: revealed ? 0.4 : 0, transition: 'opacity 1s ease 2s' }}
                  />

                  {/* Маршруты от штаб-квартиры до каждого региона */}
                  {regions
                    .filter((r) => r.id !== hub.id)
                    .map((region) => {
                      const d = arcPath(hub.coords, region.coords)
                      const isActiveLine = region.id === activeId
                      const isIntl = region.group === 'intl'
                      return (
                        <g key={`route-${region.id}`}>
                          <path
                            d={d}
                            fill="none"
                            stroke={isActiveLine ? '#FF3B30' : isIntl ? '#D4A24C' : '#F4F4F0'}
                            strokeWidth={isActiveLine ? 1.2 : 1}
                            strokeDasharray={isIntl ? '2 5' : '1 6'}
                            strokeLinecap="round"
                            style={{
                              opacity: revealed ? (isActiveLine ? 0.45 : isIntl ? 0.22 : 0.14) : 0,
                              transition: 'opacity 0.6s ease, stroke 0.4s ease',
                              transitionDelay: revealed ? '2s' : '0s',
                            }}
                          />
                          {isActiveLine && (
                            <circle key={activeId} r="3" fill="#FF3B30" filter="url(#routeGlow)">
                              <animateMotion dur="1s" fill="freeze" path={d} calcMode="ease" />
                            </circle>
                          )}
                        </g>
                      )
                  ...
                  {regions.map((region, i) => {
                    const isActive = region.id === activeId
                    const isHovered = region.id === hoveredId && !isActive
                    return (
                      <g
                        key={region.id}
                        onClick={() => setActiveId(region.id)}
                        ...
                        style={{
                          ...
                          transformOrigin: `${region.coords.x}px ${region.coords.y}px`,
                          transition: `opacity .5s ease ${2.1 + i * 0.07}s, transform .5s cubic-bezier(.34,1.56,.64,1) ${
                            2.1 + i * 0.07
                          }s`,
                        }}
                      ...
                      >
                        ...
                        <circle
                          ...
                          cy={region.coords.y}
                          r={isActive ? 7 : 4.5}
                          fill={isActive ? '#FF3B30' : region.group === 'intl' ? '#D4A24C' : '#F4F4F0'}
                          stroke={isActive ? '#FF3B30' : region.group === 'intl' ? 'rgba(212,162,76,0.5)' : 'rgba(244,244,240,0.4)'}
